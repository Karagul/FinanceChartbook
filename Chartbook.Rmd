...
source("./utils/pltlib.R")
source("./utils/theme_wsj.R")
...
```{r chargeoffs}
plt(nci_chargeoff, start_date, unit_level, recession_dates, "Million USD")
```
...
names(labels) <- d
plt(d, start_date, unit_level, recession_dates, "Index", labels=labels)
```
...
labels <- c("All", "Males")
names(labels) <- d
plt(d, start_date, unit_change, recession_dates, "Change %", labels=labels)
```

## Professional Services Employment
```{r profserivesemp}
plt(prof_services_emp, start_date, unit_change, recession_dates, "Change %")
```
